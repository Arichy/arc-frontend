...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult(`成功！短链接: <a href="${fullShortUrl}" target="_blank" rel="noopener noreferrer" class="text-blue-400 hover:text-blue-600">${fullShortUrl}</a>`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md mx-auto bg-gray-800 rounded-lg shadow-md p-8">
        ...
        {result && (
          <div 
            className={`mt-6 p-4 rounded-lg text-center ${result.startsWith('出错了') ? 'bg-red-900 border border-red-700' : 'bg-green-900 border border-green-700'}`}
